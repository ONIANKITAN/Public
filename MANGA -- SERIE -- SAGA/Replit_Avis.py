        ...
        if user_id in user_data:
            ...
            if len(check_words) <= 10:
                ...
                if matching_files:
                    ...
                    caption_message = f"Le mot ou l'un des mots dans '{message.text}' correspond à l'un des mots dans les fichiers. Cliquez sur le fichier pour effectuer une action."
...
def button_click(call):
    ...
    # Si oui, créer un nouveau fichier dans le sous-dossier
        ...
        # Vérifier si le bouton "Modifier" est cliqué
            ...
            buttons = []
            for i, line in enumerate(file_content[8:], start=8):
                parts = line.split(' : ')
                if len(parts) == 2:
            ...
            # Extraire les libellés des boutons supplémentaires
         

            # Extraire les libellés des boutons supplémentaires
            additional_button_labels = [file_content[3].split(':')[0].strip(), file_content[4].split(':')[0].strip(), file_content[5].split(':')[0].strip()]

            # Enlever les guillemets des libellés des boutons supplémentaires
            additional_button_labels = [label.replace('"', '') for label in additional_button_labels]

            # Ajouter les boutons supplémentaires à la même ligne que les autres boutons
            additional_buttons = [telebot.types.InlineKeyboardButton(label, callback_data='btn_click_handler2') for label in additional_button_labels]
            buttons[-1].extend(additional_buttons)
...
def btn_click_handler2(call):
    ...
    file_path_parts = clicked_button.split('_')
    test_file_path = os.path.join(STOCKAGE_PATH, file_path_parts[0], file_path_parts[-1])
    ...
    try:
        ...
        button_index = int(call.data.split(':')[1])
        ...
        if user_id in clicked_button_ids:
            ...
            bot.send_message(call.message.chat.id, "Vous avez déjà cliqué sur le bouton.")
            return
        ...
        for i in range(3):
            ...
            if user_id in current_ids:
                ...
                updated_clicked_button_line = f'{clicked_button_value0}"{clicked_button_number + 1}" : {{{" ".join(map(str, clicked_button_ids))}}}\n'
        ...
        # reply_markup = InlineKeyboardMarkup().add(*keyboard)

        # # Utiliser edit_message_reply_markup pour mettre à jour le message original avec les nouveaux boutons
        # bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=reply_markup)

    except FileNotFoundError:
        bot.send_message(call.message.chat.id, f"Le fichier spécifié '{test_file_path}' n'a pas été trouvé.")
    except Exception as e:
        bot.send_message(call.message.chat.id, f"Une erreur s'est produite lors de la mise à jour du fichier : {str(e)}")
