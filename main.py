...
group_ids_autorises = [
    "-1002054489996", "-1002050969919", "6039597308", "6863934525"
]  # Remplacez ces valeurs par les ID réels de vos groupes
# Liste des IDs autorisés
authorized_user_ids = {6217351762, 1234567890,
                       9876543210}  # Ajoutez tous les IDs autorisés
  ...
  with open(CLE_FILE, 'r') as clee_file:
    for line in clee_file:
      ...
      if len(parts) == 2:
        ...
        words_list = [
            word.strip().strip('"').lower() for word in words.split(',')
        ]
...
def handle_add_file(message):
  ...
  subfolders = [
      f for f in os.listdir(STOCKAGE_PATH)
      if os.path.isdir(os.path.join(STOCKAGE_PATH, f))
  ]
...
def handle_regarder(message):
  ...
  if not message.text or len(message.text.split()) < 2:
    bot.reply_to(
        message,
        "Veuillez fournir un chemin de fichier. Exemple : `/regarder Outils/Clee.txt`",
        parse_mode='Markdown')
    return
  ...
  try:
    ...
    with open(file_path, 'r', encoding='utf-8') as file_content:
      ...
      modify_button = types.InlineKeyboardButton(
          "Modifier", callback_data=f"modify:{file_path}")
      delete_button = types.InlineKeyboardButton(
          "Supprimer", callback_data=f"delete2:{file_path}")
      close_button = types.InlineKeyboardButton("Fermer",
                                                callback_data="close")
    ...
    bot.reply_to(message,
                 f"Le fichier spécifié '{file_path}' n'a pas été trouvé.")
  except Exception as e:
    bot.reply_to(
        message,
        f"Une erreur s'est produite lors de la lecture du fichier : {str(e)}")
...
def handle_message(message):
  ...
  if str(chat_id) not in group_ids_autorises and str(chat_id) != str(user_id):
    bot.send_message(
        chat_id,
        "Désolé, ce bot ne peut être utilisé que dans les groupes autorisés et en chat privé avec le bot."
    )
    return
    ...
    if user_id in user_data:
      ...
      if 'NomF' not in file_info or not file_info['NomF']:
        ...
        if file_info['subfolder'] in allowed_subfolders:
          ...
          with open(
              os.path.join(STOCKAGE_PATH, file_info['subfolder'],
                           file_info['filename']), 'a') as file:
          ...
          bot.send_message(
              chat_id,
              f"Veuillez rentrer les identifiants uniques de vos liens, qui seront associés par la suite aux parties Fixe."
          )
        ...
        if matching_files:
          ...
          delete_button = types.InlineKeyboardButton(
              "Delete", callback_data=f"delete:{file_info['filename']}")
          ignore_button = types.InlineKeyboardButton(
              "Ignore", callback_data=f"ignore:{file_info['filename']}")

          # Créer le clavier en ligne avec les boutons "Delete" et "Ignore"
          reply_markup = types.InlineKeyboardMarkup(
              [[delete_button, ignore_button]])

          # Envoyer le message avec le clavier
          bot.send_message(
              chat_id,
              f"Attention : Le Nom '{file_info['NomF']}' correspond à un mot clé dans le fichier Clée.txt. Veuillez vérifier les associations existantes.",
              reply_markup=reply_markup)
          ...
          with open(os.path.join(STOCKAGE_PATH, file_info['subfolder'],
                                 file_info['filename']),
                    'a',
                    encoding='utf-8') as file:
            # file.write(f"\n🫂GROUPE : {{Share_Group}}\n👥CANAL : {{Share_Canal}}")
            # Vérifier si le sous-dossier est "FILM"
            if file_info['subfolder'] == 'FILM' or file_info[
                'subfolder'] == 'MANGA_except':
              ...
              file.write(f"\n👉🏻Les liens de redirection :")
              numerotations_input = message.text.split()
              for i in range(0, len(numerotations_input),
                             2):  # Utiliser les mots impairs pour `{i}`
                file.write(
      ...
      check_words = message.text.strip().lower().split(
      )  # Diviser le message en mots individuels
      matching_files = []
      ...
      if len(check_words) <= 10:
        ...
        if matching_files:
          ...
          keyboard = [[
              telebot.types.InlineKeyboardButton(get_NomF_from_file(file),
                                                 callback_data=file)
  ...
  if 'modifying_file' in user_data:
    ...
    bot.send_message(message.chat.id,
                     f"Le fichier *{file_to_edit}* a été édité avec succès.",
                     parse_mode='Markdown')
...
def button_click(call):
  ...
  if clicked_button.startswith("delete:"):
    ...
    file_path_to_delete = os.path.join(STOCKAGE_PATH, subfolder_to_delete,
                                       file_to_delete)
    ...
    if os.path.exists(file_path_to_delete):
      ...
      bot.send_message(
          ...
          f"Le fichier *{file_to_delete}* a été supprimé avec succès.",
          parse_mode='Markdown')
      ...
      bot.send_message(
          call.message.chat.id,
          f"Le fichier spécifié '{file_to_delete}' n'a pas été trouvé.")
  ...
  # Vérifier si le bouton "Ignore" est cliqué
    ...
    if user_id not in user_data:
      ...
      files_in_subfolder = len([
          f
          for f in os.listdir(os.path.join(STOCKAGE_PATH, selected_subfolder))
          if f.endswith('.txt')
      ])
      new_filename = f"{selected_subfolder}_id{files_in_subfolder + 1}.txt"
      new_file_path = os.path.join(STOCKAGE_PATH, selected_subfolder,
                                   new_filename)
      ...
      bot.send_message(
          call.message.chat.id,
          f"Entrez le Nom pour le fichier {new_filename} sous ce format : 'Nom : la_Nom_entree'."
      )
    else:
      # Si l'utilisateur a déjà un fichier en cours, ignorer la requête
      bot.send_message(call.message.chat.id,
                       "Vous avez déjà un fichier en cours.")
    ...
    # Vérifier si le bouton "Supprimer" est cliqué
      ...
      if os.path.exists(file_path):
        ...
        if file_directory == os.path.abspath("Outils"):
          ...
          bot.send_message(
              call.message.chat.id,
              f"Impossible de supprimer le fichier *{file_to_delete}* dans le dossier 'Outils'.",
              parse_mode='Markdown')
        else:
          # Supprimer le fichier
          os.remove(file_path)
    ...
    # Vérifier si le bouton "Modifier" est cliqué
      ...
      bot.send_message(
          call.message.chat.id,
          "Entrez le nouveau contenu pour le fichier. Envoyez /cancel pour annuler."
      )
      ...
      if callback_data.startswith("edit:"):
        ...
        bot.delete_message(bot.callback_query.message.chat.id,
                           bot.callback_query.message.message_id)

    else:
      # Sinon, obtenir le contenu du fichier correspondant
      file_path = os.path.join(STOCKAGE_PATH,
                               clicked_button.split('_')[0], clicked_button)
      ...
      for i, line in enumerate(file_content[3:], start=3):
        ...
        if len(parts) == 2:
          ...
          link = link.strip().replace('{MANGA}', MANGA).replace(
              '{FILM}', FILM).replace('{Share_Canal}', Share_Canal).replace(
                  '{Share_Group}',
                  Share_Group).replace('{MANGA_except}', MANGA_except).replace(
                      '{SAGA}', SAGA).replace('{SERIE}', SERIE)
          ...
          if i == len(file_content) - 1:
            buttons[-1].append(
                telebot.types.InlineKeyboardButton(group_name.strip(),
                                                   url=link))
          else:
            buttons.append([
                telebot.types.InlineKeyboardButton(group_name.strip(),
